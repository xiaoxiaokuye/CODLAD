...
class EncLayer_diffusion(nn.Module):
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30):
        super(EncLayer_diffusion, self).__init__()
        self.num_hidden = num_hidden
        self.num_in = num_in
        self.scale = scale
        self.dropout1 = nn.Dropout(dropout)
        self.dropout2 = nn.Dropout(dropout)
        self.dropout3 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(num_hidden, elementwise_affine=False, eps=1e-6)
        self.norm2 = nn.LayerNorm(num_hidden, elementwise_affine=False, eps=1e-6)
        self.norm3 = nn.LayerNorm(num_hidden, elementwise_affine=False, eps=1e-6)
        ...
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
            nn.Linear(num_hidden, 9 * num_hidden, bias=True)
        )


    def forward(self, h_V, h_E, E_idx, mask_V=None, mask_attend=None, c=None):
        """ Parallel computation of full transformer layer """
        shift_norm1, scale_norm1, gate_norm1, shift_norm2, scale_norm2, gate_norm2, shift_norm3, scale_norm3, gate_norm3 = self.adaLN_modulation(c).chunk(9, dim=1)
        ...
        h_V = gate_norm1.unsqueeze(1) * modulate(h_V, shift_norm1, scale_norm1)


        dh = self.dense(h_V)
        h_V = self.norm2(h_V + self.dropout2(dh))

        h_V = gate_norm2.unsqueeze(1) * modulate(h_V, shift_norm2, scale_norm2)
        ...
        shift_norm3 = shift_norm3.unsqueeze(1)
        scale_norm3 = scale_norm3.unsqueeze(1)
        gate_norm3 = gate_norm3.unsqueeze(1)
        h_E = gate_norm3.unsqueeze(1) * modulate(h_E, shift_norm3, scale_norm3)
        return h_V, h_E


class DecLayer_diffusion(nn.Module):
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30):
        super(DecLayer_diffusion, self).__init__()
        self.num_hidden = num_hidden
        self.num_in = num_in
        self.scale = scale
        self.dropout1 = nn.Dropout(dropout)
        ...
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
            nn.Linear(num_hidden, 6 * num_hidden, bias=True)
        )

    def forward(self, h_V, h_E, mask_V=None, mask_attend=None, c=None):
        """ Parallel computation of full transformer layer """
        shift_norm1, scale_norm1, gate_norm1, shift_norm2, scale_norm2, gate_norm2 = self.adaLN_modulation(c).chunk(6, dim=1)
...
class CA_ProteinFeatures(nn.Module):
    ...
    def _orientations_coarse(self, X, E_idx, eps=1e-6):
        ...
        O = torch.stack((o_1, n_2, torch.linalg.cross(o_1, n_2)), dim=2)
